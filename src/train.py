...
if __name__ == '__main__':
    ...
    checkpoint = tf.train.Checkpoint(encoder=encoder,
                                     decoder=decoder,
                                     optimizer=optimizer)
    ckpt_manager = tf.train.CheckpointManager(checkpoint, checkpoint_path, max_to_keep=3)
    ...
    def train_step(img_tensor, target):
        ...
        hidden = tf.zeros((target.shape[0], config.units))
    ...
    for epoch in range(0, EPOCHS):
        ...
        stopping = early_stopping(loss_list, min_delta=0.01, patience=3)
        if stopping:
